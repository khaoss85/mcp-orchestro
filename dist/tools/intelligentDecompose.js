 ...
 */
export async function intelligent_decompose_story(request) {
    try {
 ...
 */
function buildAnalysisPrompt(userStory, projectConfig) {
    const techStackInfo = projectConfig?.techStack
        ?.map((t) => `- ${t.category}: ${t.framework} ${t.version || ''}`)
        .join('\n') || 'Not configured';
    ...
    const guidelinesInfo = (() => {
        ...
        if (projectConfig.guidelines.always?.length > 0) {
            parts.push('ALWAYS:\n' + projectConfig.guidelines.always.map((g) => `  - ${g}`).join('\n'));
        }
        if (projectConfig.guidelines.never?.length > 0) {
            parts.push('NEVER:\n' + projectConfig.guidelines.never.map((g) => `  - ${g}`).join('\n'));
        }
