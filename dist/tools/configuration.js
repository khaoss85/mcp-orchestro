...
import { initializeProjectTooling } from '../lib/toolRegistry.js';
...
export async function updateTechStack(params) {
    const supabase = getSupabaseClient();
    const updateData = {};
    if (params.updates.framework)
        updateData.framework = params.updates.framework;
    if (params.updates.version !== undefined)
        updateData.version = params.updates.version;
    if (params.updates.isPrimary !== undefined)
        updateData.is_primary = params.updates.isPrimary;
...
export async function removeTechStack(params) {
    const supabase = getSupabaseClient();
...
export async function updateSubAgent(params) {
    ...
    if (params.updates.enabled !== undefined)
        updateData.enabled = params.updates.enabled;
    if (params.updates.triggers)
        updateData.triggers = params.updates.triggers;
    if (params.updates.customPrompt !== undefined)
        updateData.custom_prompt = params.updates.customPrompt;
    if (params.updates.rules)
        updateData.rules = params.updates.rules;
    if (params.updates.priority !== undefined)
        updateData.priority = params.updates.priority;
    if (params.updates.configuration)
        updateData.configuration = params.updates.configuration;
    const { data, error } = await supabase
        .from('sub_agents')
        .update(updateData)
...
export async function updateMCPTool(params) {
    ...
    if (params.updates.name)
        updateData.name = params.updates.name;
    if (params.updates.command)
        updateData.command = params.updates.command;
    if (params.updates.enabled !== undefined)
        updateData.enabled = params.updates.enabled;
    if (params.updates.whenToUse)
        updateData.when_to_use = params.updates.whenToUse;
    if (params.updates.priority !== undefined)
        updateData.priority = params.updates.priority;
    if (params.updates.url !== undefined)
        updateData.url = params.updates.url;
    if (params.updates.apiKey !== undefined)
        updateData.api_key = params.updates.apiKey;
