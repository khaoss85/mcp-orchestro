...
function dbRowToResourceNode(row) {
    return {
        id: row.id,
        type: row.type,
        name: row.name,
...
function dbRowToResourceEdge(row) {
    return {
        id: row.id,
...
export async function saveDependencies(taskId, resources) {
    ...
    return withRetry(async () => {
        try {
            ...
            const conflicts = (conflictData || []).map((c) => ({
...
export async function getTaskDependencyGraph(taskId) {
    ...
    return withRetry(async () => {
        ...
        `)
            ...
            for (const row of data) {
                const resourceNode = row.resource_nodes;
...
export async function getResourceUsage(resourceId) {
...
export async function getTaskConflicts(taskId) {
