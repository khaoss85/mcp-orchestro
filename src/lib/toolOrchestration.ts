...
export class MCPToolOrchestrator implements ToolRecommendationEngine {
  ...
  async analyzeTaskForTools(
    ...
    return (recommendations || [])
      .filter((rec: any) => rec.confidence >= this.config.minimumConfidence)
      .map((rec: any) => ({
   ...
   * Get tool by name
   */
  async getTool(name: string, projectId?: string): Promise<MCPToolInfo | null> {
    const supabase = getSupabaseClient();
   ...
   */
  async getToolStats(toolName: string, projectId?: string): Promise<ToolUsageStats | null> {
    const supabase = getSupabaseClient();
    ...
    const successRate =
      data.usage_count > 0 ? data.success_count / data.usage_count : 0;
   ...
   */
  async getEnabledTools(projectId: string): Promise<MCPToolInfo[]> {
    const supabase = getSupabaseClient();
 ...
 */
export async function enrichTaskContextWithTools(
  taskId: string,
