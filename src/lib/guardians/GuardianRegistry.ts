...
export class GuardianRegistry implements GuardianFactory {
  private guardians: Map<string, IGuardian> = new Map();
   ...
   */
  async loadGuardiansForProject(projectId: string): Promise<void> {
    ...
    for (const row of guardianRows || []) {
      try {
        const guardian = this.createGuardian(row.guardian_type as GuardianType, {
          ...row.configuration,
          enabled: row.enabled,
   ...
   */
  async runAllGuardians(context: GuardianRunContext): Promise<GuardianValidationResult[]> {
    const allResults: GuardianValidationResult[] = [];
  ...
  private async saveValidations(
    guardianId: string,
    taskId: string,
    results: GuardianValidationResult[]
  ): Promise<void> {
   ...
   */
  async getGuardianReport(taskId: string): Promise<GuardianReport[]> {
    const supabase = getSupabaseClient();
   ...
   */
  static async initializeDefaultGuardians(projectId: string): Promise<void> {
    const supabase = getSupabaseClient();
 ...
 */
export async function runGuardiansOnTask(
  taskId: string,
  context: Omit<GuardianRunContext, 'taskId'>
): Promise<GuardianValidationResult[]> {
  const fullContext: GuardianRunContext = {
    taskId,
    ...context,
  };
